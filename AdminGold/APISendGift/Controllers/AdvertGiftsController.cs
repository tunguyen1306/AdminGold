...
using APISendGift.Models;

namespace APISendGift.Controllers
{
    public class AdvertGiftsController : ApiController
    {
        private ApiGiftEntities db = new ApiGiftEntities();
        ...
        // GET: api/AdvertGifts/5
        public IHttpActionResult GetAdvertByType(int Typeid)
        {
            var tblAdvertGift = db.tblAdvertGifts.Where(x=>x.IdTypeAdvertGift== Typeid).ToList();
            if (tblAdvertGift == null)
            {
                return NotFound();
            }
